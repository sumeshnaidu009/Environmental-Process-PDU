...
class GPS:
    ...
    def lla2Vxyz(self,lla):
        lat = deg2rad(lla[0])
        lon = deg2rad(lla[1])
        alt = lla[2]

        N = self.wgs84.a / sqrt(1 - (self.wgs84.e * sin(lat))**2)
        N1 = (self.wgs84.a) * ((1-((self.wgs84.e) * (sin(lat)))**2)**(-3/2)) * ((self.wgs84.e) * sin(lat) * cos(lat))

        x = (N + alt) * cos(lat) * cos(lon)
        vx = (cos(lat) * cos(lon) * N1) + ((N+alt) * cos(lon) * (-sin(lat))) + ((N+alt) * cos(lat) * (-sin(lon)))
        y = (N + alt) * cos(lat) * sin(lon)
        vy = (cos(lat) * sin(lon) * N1) + ((N+alt) * sin(lon) * (-sin(lat))) + ((N+alt) * cos(lat) * (cos(lon)))
        z = (N * (1 - self.wgs84.e**2) + alt) * sin(lat)
        vz = ((1-(self.wgs84.e)**2) * ((N * cos(lat)) + (N1 * sin(lat)))) + (alt * cos(lat))
        return (vx,vy,vz)
